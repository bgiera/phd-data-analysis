...
def  P3F1():
    """
	Plotting routines for publication #3. Just need to get this done... 
		Modularity for another time...
Input: 
      XXXXfilenameS - A vector of all the filenames to be included in the plots
...
#					print filename
				...
				if sigWCA==0.5:
					...
					sig_HS = 0.904801567015
				elif sigWCA==0.7539484789:
					sig_HS = 0.68217376515732153
				elif sigWCA==0.94007:
					sig_HS = 0.85057680910382316
				elif sigWCA==1.244741513:
					sig_HS = 1.1262440714910646
				elif sigWCA==1.81712:
					sig_HS = 1.644133023454359	
				elif sigWCA==1.846399291:
					sig_HS = 1.6706249718322481						
				elif sigWCA==2.0:
					sig_HS = 1.80960313403
				elif sigWCA==2.311031:
					sig_HS = 2.0910244702203213
				elif sigWCA==2.4278:
					sig_HS = 2.1966772443990998
				elif sigWCA==2.7324:
					sig_HS = 2.4722798017118794
				elif sigWCA==3.0:
					sig_HS = 2.714440470105
				elif sigWCA==3.4665469857:
					sig_HS = 3.13653714479
				elif sigWCA==3.5442:
					sig_HS = 3.2067977138146837
				elif sigWCA==3.634246:
					sig_HS = 3.28827147572
				elif sigWCA==3.73422454:
					sig_HS = 3.37873221538
				elif sigWCA==3.760284664:
					sig_HS = 3.40231145641
				elif sigWCA==3.7697423947:
					sig_HS = 3.4108688259675679	
				elif sigWCA==4.5:
					sig_HS = 4.0716070515676543
				elif sigWCA==5.0:
					sig_HS = 4.52400783508
				elif sigWCA==5.4:
					sig_HS = 4.88592846188
				elif sigWCA==5.46479:
					sig_HS = 4.94455055541
				elif sigWCA==5.539197872:
					sig_HS = 5.01187491459
				elif sigWCA==5.61513:
					sig_HS = 5.08057842299
				elif sigWCA==5.84609:
					sig_HS = 5.28955139291
				elif sigWCA==6.0:
					sig_HS = 5.42880940209
				elif sigWCA==7.0:
					sig_HS = 6.33361096911
				elif sigWCA==7.08841:
					sig_HS = 6.41360447565
				elif sigWCA==7.28341:
					sig_HS = 6.59004078121
				elif sigWCA==8.0:
					sig_HS = 7.23841253612
				elif sigWCA==9.6:
					sig_HS = 8.68609504334
...
#					sig_HS = 1E9 ##THis is turned off for faster code
					if i==1:
						print '\t**Hard sphere diameters are being calculated. This takes time.'	
					print '(Co ion WCA,HS) = ', (sigWCA,sig_HS)
#					print filename,sig_HS
...
#					sig_HS = 1E9 ##THis is turned off for faster code
		if i==1:
			print '\t**Hard sphere diameters are being calculated. This takes time.'	
		print '(ion WCA,HS) = ', (sigWCA,sig_HS)
#					print filename,sig_HS

	sigLast = sigWCA
	if sigLast>=sigFirst:  #Counterions are BIGGER and negative
		sigC_S.append(sigLast)
		sigCHS_S.append(sig_HS)
		sigCo_S.append(sigFirst)
		sigCoHS_S.append(sig_FirstHS)
		qcS.append(-1)		
	else:
		sigC_S.append(sigLast)
		sigCHS_S.append(sig_HS)
...
#	print filename,'\t',E_tot[-1],'\t',Psi_tot[-1],'\t',L_bin,'\t',zpos[-1]
	...
	N_plusS.append(N_plus[:-2])
	  ...
	  #This Co notation here on excess is probably correct -- 03/13/14 15:07:52 
	muex_EV=z_50[:,6]
	muexEV2tot_S.append(muex_EV[-1])
	muex_EVz = muex_EV[:-1].tolist()
	muexEV2_S.append(muex_EVz)

	##This switch guaruntees the free charge should have the "proper" sign... - 03/18/14 12:22:31 
    temp = originalN_plusS
    originalN_plusS = originalN_minusS
    originalN_minusS = temp
    		##but it fucks with co- and counter-ion labeling. This is corrected below. - 03/18/14 12:22:39 

    print qcS,len(qcS),len(filenameS)
    NDSigmaS,SigRefS,zetaS=[],[],[]
    zEffS,SigEffS,VEffS,zetaS=[],[],[],[]
    bulkVoltS=[]
    bulkEVS=[]
    NDrhoCoS,NDrhoCounterS =[],[] ##Co = plus and counter = minus, likely...
    EVCS,EVCoS=[],[]
    print 'Assembling plottable profiles...'
    jj=-1
    for (Nm,Np,n0,Sigma_s,area,lam_D,L_bin,z_positions,L_z,Bjerrum,filename,sigWCA,V,muex,Npeff,Nmeff,Phi,muex2,qc) in zip(originalN_plusS,originalN_minusS,n0s,SIGMAS,areaS,LDs,L_binS,z_originalS,L_zS,BjerrumS,filenameS,sigWCA_S,Volts,muexEV_S,N_plusS,N_minusS,PhiWCAtot_S,muexEV2_S,qcS):
	jj+=1
	dielectric=(4*np.pi*Bjerrum)**-1
	...
	SigEff=[]
	VEff=[]
    	volt_correction = Sigma_s/(dielectric*temperature/(valency*lam_D))  - abs(eff) ##Oriignal
#    	volt_correction = Sigma_s/(dielectric*temperature/(valency*lam_D))  + eff  #Gives the same answer, really
	Nf_for_Eff=[]
	zEff = []
	rhoplus,rhominus,EVexcess,EV2excess = [],[],[],[]
	for (y1,y2,z,volt,p,m,x,x2) in zip(Nf[0:len(Nf)-1],Nf[1:len(Nf)],z_pos,V[:len(Nf)],Npeff[:len(Nf)],Nmeff[:len(Nf)],muex[:len(Nf)],muex2[:len(Nf)]):  #This is the original line of code -- 07/30/13 16:51:36 
			zEff.append(z)
			SigEff.append(Integrand) #There's something wrong with the first value assigned to this function
...
#			else:
	...
	VEff = np.array(VEff)
	bulkvolt = np.mean(VEff[-5:])
	bulkVoltS.append(bulkvolt)
#	print 'VOLTAGE  is NOT expressed relative to the bulk...'
#	print VEff[:10]
	print 'Voltage is now being expressed relative to bulk'
	VEff = VEff - bulkvolt
#	print VEff[:10]	
	VEffS.append(VEff)
	zetaS.append(VEff[0])	
	...
	print filename,qc
	if qc==-1: #Counterions are SMALLER (sign of qc is likely correct)
		##Verified -- Co-ions are BIGGER
		rhoc = np.array(rhoplus)/(area*L_bin*n0) 
		rhoco = np.array(rhominus)/(area*L_bin*n0)		
		EVCoexcess = np.array(EVexcess) - np.mean(EVexcess[-5:])
		EVCexcess = np.array(EV2excess) - np.mean(EV2excess[-5:])	
	else:	#Counterions are BIGGER
		##Verified #print 'SMALLER CO-ions'
		rhoco = np.array(rhoplus)/(area*L_bin*n0)
		rhoc = np.array(rhominus)/(area*L_bin*n0)		
		EVCexcess = np.array(EVexcess) - np.mean(EVexcess[-5:])
		EVCoexcess = np.array(EV2excess) - np.mean(EV2excess[-5:])
#		print 'Trying something new...'
#		EVCoexcess = np.array(EVexcess) - np.mean(EVexcess[-5:])
#		EVCexcess = np.array(EV2excess) - np.mean(EV2excess[-5:])

#		print np.mean(EV2excess[-5:]),np.mean(EVexcess[-5:])
	NDrhoCoS.append(rhoco)
	NDrhoCounterS.append(rhoc)
	EVCoS.append(EVCoexcess)
	EVCS.append(EVCexcess)
	bulkEV = (np.mean(EVexcess[-5:]) + np.mean(EV2excess[-5:]))*0.5
	bulkEVS.append(bulkEV)


#	ctest = np.log(rhoc)+qc*(VEff-bulkvolt)+EVCexcess
#	cotest = np.log(rhoco)-qc*(VEff-bulkvolt)+EVCoexcess
#	print 'ln(co) + volt + EVco = \n',cotest
#	print 'ln(c) - volt + EVc = \n',ctest


#	if '1312' in filename:
#		print '\n\n',filename,'  This should work for all now... maybe'
#		print 'sigCo , sigC, ratio Co/C  = ',sigCo_S[jj],sigC_S[jj],sigCo_S[jj]/sigC_S[jj]
##	#	print 'EVcoTest = ',EVCoexcess[:6]
##		print 'EVcTest = ',EVCexcess[:6]
##		print 'CoTest = ',rhoco[:10]
##		print 'CTest = ',rhoc[:10]
##		ctest = np.log(rhoc)+qc*(VEff-bulkvolt)+EVCexcess
##		cotest = np.log(rhoco)-qc*(VEff-bulkvolt)+EVCoexcess
##		print 'ln(co) + volt + EVco = \n',cotest
##		print 'ln(c) - volt + EVc = \n',ctest
##		print 'Summing chemical potentials = \n',ctest+cotest,'\n'
##		for (c1,co1) in zip(ctest,cotest):
###			print c1,'\t',co1,'\t',c1+co1
##			print c1+co1
#		for (z,v,rc,evc,rco,eco) in zip(zEff,VEff,rhoc,EVCexcess,rhoco,EVCoexcess,):
#			print z,'\t',v,'\t',rc,'\t',evc,'\t',rco,'\t',eco

#    print np.mean(n0s)
#    print fail

#    i=1
#    print '\n\nsigP = ',np.mean(sigCoHS_S[i*10:10*(i+1)])
#    print 'sigM = ',np.mean(sigCHS_S[i*10:10*(i+1)])
#    print 'rhoPM = ',np.mean(n0s[i*10:10*(i+1)])
#    print 'rat = %1.3f' % (np.mean(sigCo_S[i*10:10*(i+1)])/np.mean(sigC_S[i*10:10*(i+1)]))

    qcS = -1*np.ones(len(qcS))
    if len(filenameS)<=11:
#	    print 'MuTot test versus Distance'
#	    fig=plt.figure()  
#	    ax1=fig.add_subplot(211)
#	    ax2=fig.add_subplot(212)
#	    rat_label = np.round(sigC_S[0]/sigCo_S[0],2)
#	    if rat_label>=1. and rat_label!=1.5:
#		    rat_label = str(int(rat_label))
#	    elif rat_label==1.5:
#    		    rat_label = str(np.round(rat_label,1))
#	    elif rat_label<1.:
#    		    rat_label = str(np.round(rat_label,2))
...
#	    AvgPhi = (np.pi/6.)*np.mean(n0s)*(sigC_S[0]**3+sigCo_S[0]**3)
#	    if AvgPhi<=0.1:
#	    	Phix=0.05
#	    elif AvgPhi>0.1 and AvgPhi<=0.2:
#	    	Phix=0.17
#    	    else:
#    	    	Phix=0.371
#	    ax1.plot([-1,-1],[-1,-1],color = 'k', ls = 'None',lw = 1.0,marker = 'None',label =r'$\sigma_{\rm c} / \sigma_{\rm co} = $'+"""$"""+rat_label+"""$""")               
#	    ax2.plot([-1,-1],[-1,-1],color = 'k', ls = 'None',lw = 1.0,marker = 'None',label ='  '+"""$="""+rat_label+"""$""")               
#	    j=-1
#    	    offset = np.array(range(len(filenameS)))
##            markerS=['v','o','>','s','^','d','<','*','h','D','H','p','3','1','4','2']
#	    for (z,c,co,v,Sig,lam_D,zeta,bulkvolt,evco,evc,qc) in zip(zEffS,NDrhoCounterS,NDrhoCoS,VEffS,NDSigmaS,LDs,zetaS,bulkVoltS,EVCoS,EVCS,qcS):
#	    	j+=1

#		mucotot = np.log(co)-qc*v+evco
#		muctot = np.log(c)+qc*v+evc

#		mark='None'

#		nd_z = sigC_S[j]
#		zPM = z/nd_z-(1+0.299602925)/nd_z
#    	
#		col = ROYGBIV_map(Sig,np.max(NDSigmaS)*1.05)
#		if j ==0:
#			labelx = """$\~\Sigma = """+str(np.round(Sig,1))+"""$"""
#		else:
#			labelx = """$"""+str(np.round(Sig,1))+"""$"""			

#	    	ax1.plot(zPM,mucotot+ offset[j],color=col,ls='-',marker = mark,label =labelx,ms = 2.5)
#	    	ax2.plot(zPM,muctot+ offset[j],color=col,ls='-',marker = mark,label =labelx,ms = 2.5)
#	    	
#	    ax1.set_ylabel(r'$ \mu_{\rm co}^{\rm tot} + {\rm offset} $',fontsize=10.) # = (n_+ - n_-) / 2 \rho^{\rm B}
#	    ax2.set_ylabel(r'$ \mu_{\rm c}^{\rm tot} + {\rm offset} $',fontsize=10.) 
#	    ax2.set_xlabel(r'$(z-\delta_{\rm w}) / \sigma_{\rm c}$',fontsize=10.)
#	    plt.setp(ax1.get_xticklabels(), fontsize=8.)
#	    plt.setp(ax1.get_yticklabels(), fontsize=8.)
#	    plt.setp(ax2.get_xticklabels(), fontsize=8.)
#	    plt.setp(ax2.get_yticklabels(), fontsize=8.)
#	    ax1.axis(xmin=-1/nd_z)
#	    ax2.axis(xmin=-1/nd_z)
#	    ax2.legend(loc='best',numpoints=1,prop=dict(size=8.),columnspacing=0.1,borderpad=0.25,labelspacing=0.1,ncol=3,handletextpad=0.07)#,markerscale=0.75)     
#	    fig.set_size_inches(3.37,4.5)
#	    plt.savefig('F1_mutest_'+str(Phix)+'_'+rat_label+'.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=False,bbox_inches='tight')   
##	    if float(rat_label)<1:
#	    ax1.set_xlim(-(1+0.299602925)/nd_z,25.) 				
#	    ax2.set_xlim(-(1+0.299602925)/nd_z,25.) 
#	    plt.savefig('F1_mutest_'+str(Phix)+'_'+rat_label+'_zoom.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=False,bbox_inches='tight')   
#	    plt.close()#	##Asym

#	    print fail

#	    print 'Plotting Rhof and Volt versus Distance'
#	    fig=plt.figure()  
#	    ax1=fig.add_subplot(211)
#	    ax2=fig.add_subplot(212)
#	    BMCSLtheory='yes'
...
#    	    offset = np.array(range(len(filenameS)))
#            markerS=['v','o','>','s','^','d','<','*','h','D','H','p','3','1','4','2']
#	    for (z,c,co,v,Sig,lam_D,zeta,bulkvolt) in zip(zEffS,NDrhoCounterS,NDrhoCoS,VEffS,NDSigmaS,LDs,zetaS,bulkVoltS):
#	    	j+=1
...
##		offset[j]=offset[j]*0
#		#THIS MMA FORMAT WORKs
##		print '\n\nsigM = ',sigCoHS_S[j]
##		print 'sigP = ',sigCHS_S[j]
##		print 'rhoPM = ',n0s[j]
###		print 'rat = '+rat_label
##		print 'zeta = %1.2f' % zeta
##		MMA_filename = '"BMCSL_Full_'+str(Phix)+'_'+rat_label+'_'+str(np.round((zeta-bulkvolt),2))+'.txt"'
##		print 'filename = ',MMA_filename
#		
#	    	ax1.plot(zPM,rhof+ offset[j],color=col,ls='None',marker = markerS[j],label =labelx,ms = 2.5)
#	    	ax2.plot(zPM,v+ offset[j],color=col,ls='None',marker = markerS[j],label =labelx,ms = 2.5)
#	    	if BMCSLtheory=='yes':
#			if np.round(zeta,2)>=0.25:
#				  BMCSL_file='BMCSL_Full_'+str(Phix)+'_'+rat_label+'_'+str(np.round(zeta,2))+'.txt'
#				  print '\t\tPlotting volt and free theory for: ',BMCSL_file
#				  MMA_BMCSL=[[float(x) for x in line.split()] for line in file(BMCSL_file,"r").readlines()]
#				  MMAz = []
#				  MMA_free,MMA_volt = [],[]
#				  for line in MMA_BMCSL:
#				  	MMAz.append(line[0])
#				  	free = (-line[1] + line[2])/2
#				  	MMA_free.append(free)
#					volt = line[4]
#					MMA_volt.append(volt)
#				  MMAz = np.array(MMAz)*lam_D/nd_z-(1+0.299602925)/nd_z
#		       		  ax1.errorbar(MMAz,np.array(MMA_free)+offset[j],yerr=None,color=col,ls='-',lw=1.0)	
#		       		  ax2.errorbar(MMAz,np.array(MMA_volt)+ offset[j],yerr=None,color=col,ls='-',lw=1.0)
#		       		  
#	       		else:
#		       		  ax1.errorbar(zPM,np.zeros(len(zPM))+offset[j],yerr=None,color=col,ls='-',lw=1.0)	
#		       		  ax2.errorbar(zPM,np.zeros(len(zPM))+ offset[j],yerr=None,color=col,ls='-',lw=1.0)
#	    	else:
#			print '\n\nsigM = ',sigCoHS_S[j]
#			print 'sigP = ',sigCHS_S[j]
#			print 'rhoPM = ',n0s[j]
#	#		print 'rat = '+rat_label
#			print 'zeta = %1.2f' % zeta
#			MMA_filename = '"BMCSL_Full_'+str(Phix)+'_'+rat_label+'_'+str(np.round(zeta,2))+'.txt"'
#			print 'filename = ',MMA_filename
#	    	
#	    ax1.set_ylabel(r'$- \rho / 2 e n^{\rm B} + {\rm offset} $',fontsize=10.) # = (n_+ - n_-) / 2 \rho^{\rm B}
#	    ax2.set_ylabel(r'$ (\phi - \phi^{\rm B}) / \phi^{\rm T} + {\rm offset}$',fontsize=10.) 
#	    ax2.set_xlabel(r'$(z-\delta_{\rm w}) / \sigma_{\rm c}$',fontsize=10.)
#	    plt.setp(ax1.get_xticklabels(), fontsize=8.)
#	    plt.setp(ax1.get_yticklabels(), fontsize=8.)
#	    plt.setp(ax2.get_xticklabels(), fontsize=8.)
#	    plt.setp(ax2.get_yticklabels(), fontsize=8.)
#	    ax1.axis(ymin=-0.25)
#	    ax1.set_xlim(-(1+0.299602925)/nd_z,15.) 
#	    ax2.axis(ymin=-0.25)
#	    ax2.set_xlim(-(1+0.299602925)/nd_z,15.) 
##	    if Phix ==0.05:
##		ax1.set_xlim(0,15.) 
##		ax1.set_ylim(0,40.0)
##	    elif Phix==0.17:
...
##		ax1.set_ylim(0,16.0)
#	    ax2.legend(loc='best',numpoints=1,prop=dict(size=8.),columnspacing=0.1,borderpad=0.25,labelspacing=0.1,ncol=3,handletextpad=0.07)#,markerscale=0.75)     
#	    fig.set_size_inches(3.0,4.5)
#	    plt.savefig('F1_RhoVoltZ_'+str(Phix)+'_'+rat_label+'.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=False,bbox_inches='tight')   
#	    plt.close()#	##Asym


	    print 'Plotting ideal and EV excess components versus Distance'
	    fig=plt.figure()  
...
#	    BMCSLtheory='no'
	    ...
	    ax3.plot([-1,-1],[-1,-1],color = 'k', ls = 'None',lw = 1.0,marker = 'None',label ='.')      #label ='  '+"""$   ="""+rat_label+"""$"""         
	    j=-1
    	    offset = np.array(range(len(filenameS)))
            markerS=['v','o','>','s','^','d','<','*','h','D','H','p','3','1','4','2']
	    for (z,c,co,Sig,lam_D,EVco,EVc,zeta) in zip(zEffS,NDrhoCounterS,NDrhoCoS,NDSigmaS,LDs,EVCoS,EVCS,zetaS):
	    	j+=1
		...
		if np.round(zeta,2)>=0.25:
		    	ax1.plot(zPM,ideal_co +offset[j],color=col,ls='None',marker = markerS[j],label =labelx,ms = 2.5)
		    	ax2.plot(zPM,ideal_c +offset[j],color=col,ls='None',marker = markerS[j],label =labelx,ms = 2.5)
		    	ax3.plot(zPM,EVco +offset[j],color=col,ls='None',marker = markerS[j],label =labelx,ms = 2.5)
		    	ax4.plot(zPM,EVc +offset[j],color=col,ls='None',marker = markerS[j],label =labelx,ms = 2.5)
#		    	print np.max(EVco),np.max(EVc)
			...
			if np.round(zeta,2)>=0.25:
				  BMCSL_file='BMCSL_Full_'+str(Phix)+'_'+rat_label+'_'+str(np.round(zeta,2))+'.txt'
				  print '\t\tPlotting theory for mu components with: ',BMCSL_file
				  MMA_BMCSL=[[float(x) for x in line.split()] for line in file(BMCSL_file,"r").readlines()]
				  MMAz = []
				  MMA_co,MMA_c = [],[]
				  MMA_evco,MMA_evc = [],[]
				  for line in MMA_BMCSL:
					...
					MMA_c.append(c)
					evco = line[6]
					MMA_evco.append(evco)
					evc = line[7]
					MMA_evc.append(evc)
				  MMA_co,MMA_c = np.array(MMA_co),np.array(MMA_c)
				  MMA_evco,MMA_evc = np.array(MMA_evco),np.array(MMA_evc)
				  MMAz = np.array(MMAz)*lam_D/nd_z-(1+0.299602925)/nd_z
		       		  ax1.errorbar(MMAz,np.log(MMA_co)+offset[j],yerr=None,color=col,ls='-',lw=1.0)	
		       		  ax2.errorbar(MMAz,np.log(MMA_c)+ offset[j],yerr=None,color=col,ls='-',lw=1.0)
		       		  ax3.errorbar(MMAz,MMA_evco + offset[j],yerr=None,color=col,ls='-',lw=1.0)
		       		  ax4.errorbar(MMAz,MMA_evc + offset[j],yerr=None,color=col,ls='-',lw=1.0)
...
#			print 'filename = ',MMA_filename
	    ...
	    ax1.set_ylabel(r'$\ln (n_{\rm co}/n^{\rm B})$',fontsize=10.)
            ax2.set_ylabel(r'$\ln (n_{\rm c}/n^{\rm B})$',fontsize=10.)
	    ax3.set_ylabel(r'$(\mu^{\rm EV}_{\rm co}-\mu^{\rm EV}_{\rm B}) / k_{\rm B}T$',fontsize=9.)
            ax4.set_ylabel(r'$(\mu^{\rm EV}_{\rm c}-\mu^{\rm EV}_{\rm B}) / k_{\rm B}T$',fontsize=9.)
    	    ax4.set_xlabel(r'$(z-\delta_{\rm w}) / \sigma_{\rm c}$',fontsize=10.)
	    ...
	    ax1.set_xlim(-(1+0.299602925)/nd_z,15)
	    ax2.set_xlim(-(1+0.299602925)/nd_z,15)
	    ax3.set_xlim(-(1+0.299602925)/nd_z,15)	
	    ax4.set_xlim(-(1+0.299602925)/nd_z,15)
	    ...
	    if Phix ==0.05:
			...
			ax1.set_ylim(-10,0.5)
			ax2.set_ylim(-0.25,3.5)  #Default options appear good enough
...
#    			ax4.yaxis.set_major_locator(MaxNLocator(5)) 
			...
			ax1.set_ylim(-11,0.5)
			ax2.set_ylim(-0.25,2.5)  #Default options appear good enough
#    			ax2.yaxis.set_major_locator(MaxNLocator(6)) 
			ax3.set_ylim(-0.25,4)  
			ax4.set_ylim(-0.5,14) 
	    elif Phix==0.371:
...
#    			ax4.yaxis.set_major_locator(MaxNLocator(5)) 
			...
			ax1.set_ylim(-12,0.5)
			ax2.set_ylim(-2,2.5)  #Default options appear good enough
    			ax2.yaxis.set_major_locator(MaxNLocator(6)) 
			ax3.set_ylim(-1,16)  
			ax4.set_ylim(-1,45) 
	    ax1.legend(loc='best',numpoints=1,prop=dict(size=8.),columnspacing=0.05,borderpad=0.25,labelspacing=0.07,ncol=3,handletextpad=0.07,handlelength = 1.25)#,markerscale=0.75)     
	    fig.set_size_inches(3.0,5.0)
	    plt.savefig('F1_muComponentsZ_'+str(Phix)+'_'+rat_label+'.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=False,bbox_inches='tight')   
	    plt.close()#	##Asym
    ...
    multiplier = int(len(filenameS)/11.)
    print multiplier

    if len(filenameS)==88:
	    groupColorS = ['red','orange','yellow','green','blue','cyan','violet','grey']
    	    markerS = ['^','o','p','<','s','d','h','v']
#	    groupLabelS = [r'$\sigma_{\rm c}/\sigma_{\rm co}=5$',r'$4$',r'$3$',r'$2$',r'$1.5$',r'$1$',r'$0.50$',r'$0.25$']
#	    TheoryNames = ['BMCSL_QV_0.05_5_15.txt','BMCSL_QV_0.05_4_15.txt','BMCSL_QV_0.05_3_15.txt','BMCSL_QV_0.05_2_15.txt','BMCSL_QV_0.05_1.5_15.txt','BMCSL_QV_0.05_1_15.txt','BMCSL_QV_0.05_0.50_15.txt','BMCSL_QV_0.05_0.25_15.txt']

		##R_lows_0.05.txt
	    groupLabelS,TheoryNames = [r'$\sigma_{\rm c}/\sigma_{\rm co}=5$',r'$3$',r'$2$',r'$1.5$',r'$1$',r'$0.67$',r'$0.50$',r'$0.20$'],['BMCSL_QV_0.05_5_15.txt','BMCSL_QV_0.05_3_15.txt','BMCSL_QV_0.05_2_15.txt','BMCSL_QV_0.05_1.5_15.txt','BMCSL_QV_0.05_1_15.txt','BMCSL_QV_0.05_0.67_15.txt','BMCSL_QV_0.05_0.50_15.txt','BMCSL_QV_0.05_0.20_15.txt']
		##R_lows_0.05_2.txt
#	    groupLabelS,TheoryNames = [r'$\sigma_{\rm c}/\sigma_{\rm co}=5$',r'$3$',r'$2$',r'$1.5$',r'$1$',r'$0.67$',r'$0.33$',r'$0.20$'],['BMCSL_QV_0.05_5_15.txt','BMCSL_QV_0.05_3_15.txt','BMCSL_QV_0.05_2_15.txt','BMCSL_QV_0.05_1.5_15.txt','BMCSL_QV_0.05_1_15.txt','BMCSL_QV_0.05_0.67_15.txt','BMCSL_QV_0.05_0.33_15.txt','BMCSL_QV_0.05_0.20_15.txt']
	    Phix=0.05
    else:
	    groupColorS = ['red']*multiplier
	    groupLabelS = [r'$test$']*multiplier
	    markerS = ['^']*multiplier	

    if len(filenameS) == 55 and np.mean(PhiBulkS)<=0.25:
    	groupColorS = ['orange','yellow','blue','violet','purple']	
	markerS = ['o','p','s','h','D']
#        TheoryNames,groupLabelS = ['BMCSL_QV_0.17_3.09_19.5.txt','BMCSL_QV_0.17_2_26.8.txt','BMCSL_QV_0.17_1_27.txt','BMCSL_QV_0.17_0.50_27.txt','BMCSL_QV_0.17_0.324_27.txt'],[r'$\sigma_{\rm c}/\sigma_{\rm co}=3.09$',r'$2$',r'$1$',r'$0.5$',r'$0.324$']

	groupLabelS = [r'$\sigma_{\rm c}/\sigma_{\rm co}=3$',r'$2$',r'$1$',r'$0.5$',r'$0.33$']
        TheoryNames = ['BMCSL_QV_0.17_3_30.txt','BMCSL_QV_0.17_2_26.8.txt','BMCSL_QV_0.17_1_27.txt','BMCSL_QV_0.17_0.50_27.txt','BMCSL_QV_0.17_0.33_30.txt']

#        TheoryNames = ['BMCSL_QV_0.17_3_30.txt','BMCSL_QV_0.17_2_26.8.txt','BMCSL_QV_0.17_1_27.txt','BMCSL_Full_0.17_0.5_6.17.txt','BMCSL_QV_0.17_0.33_30.txt']
#        TheoryNames = ['BMCSL_QV_0.17_3_30.txt','BMCSL_QV_0.17_2_26.8.txt','BMCSL_QV_0.17_1_27.txt','BMCSL_Full_0.17_0.5_6.17.txt','BMCSL_QV_0.17_0.50_27.txt']

	Phix=0.17
    elif len(filenameS) == 55 and np.mean(PhiBulkS)>0.25:
    	groupColorS = ['orange','yellow','blue','violet','purple']
        groupLabelS = [r'$\sigma_{\rm c}/\sigma_{\rm co}=3$',r'$2$',r'$1$',r'$0.5$',r'$0.33$']	
	markerS = ['o','p','s','h','D']
        TheoryNames = ['BMCSL_QV_0.371_3_30.txt','BMCSL_QV_0.371_2_30.txt','BMCSL_QV_0.371_1_27.txt','BMCSL_QV_0.371_0.50_27.txt','BMCSL_QV_0.371_0.33_27.txt']
	Phix=0.371   
    ...
    ax1.plot([-1,-1],[-1,-1],color = 'k', ls = '-',lw = 1.0,marker = 'None',label =r'${\rm BMCSL}$')               
    j=-1
    for i in range(multiplier):
    	j+=1
    	if j>len(groupColorS):
    		j=0
    	plotzetaS = np.array(zetaS[i*11:11*(i+1)])
    	plotNDSigmaS = NDSigmaS[i*11:11*(i+1)]	

#    	plotzetaS = plotzetaS - np.array(bulkVoltS[i*11:11*(i+1)])
...
#	print '\n\nsigM = ',np.mean(sigCoHS_S[i*10:10*(i+1)])
#	print 'sigP = ',np.mean(sigCHS_S[i*10:10*(i+1)])
#        print 'rhoPM = ',np.mean(n0s[i*10:10*(i+1)])
#        print 'rat = %1.3f' % (np.mean(sigC_S[i*10:10*(i+1)])/np.mean(sigCo_S[i*10:10*(i+1)]))
		

	#To verify everything is plotted correctly
#        print 'c/co rat = %1.3f' % (np.mean(sigC_S[i*11:11*(i+1)])/np.mean(sigCo_S[i*11:11*(i+1)]))
#        print groupLabelS[j],TheoryNames[i],groupColorS[j]
#        print 'Max volt = ',np.max(plotzetaS),'\n'

	mark = markerS[j]
	if len(filenameS)==88 or len(filenameS)==55:
	    	ax1.plot(plotzetaS,plotNDSigmaS,color=groupColorS[j],ls='None',marker = mark,label =groupLabelS[j])
	else:
	    	ax1.plot(plotzetaS,plotNDSigmaS,color=groupColorS[j],ls='-',marker = mark)#,label =groupLabelS[i])
      
	if 'BMCSL_QV_' in TheoryNames[i]:	
		QV_BMCSL=np.array([[float(x) for x in line.split()] for line in file(TheoryNames[i],"r").readlines()])
		ax1.errorbar(QV_BMCSL[:,0],QV_BMCSL[:,1],yerr=None,ls='-',lw=1.0,color=groupColorS[j],marker='None')#,label =groupLabelS[i])
	else:
	   	QV_BMCSL=np.array([[float(x) for x in line.split()] for line in file(TheoryNames[i],"r").readlines()])
		ax1.errorbar(QV_BMCSL[:,4],QV_BMCSL[:,5],yerr=None,ls='-',lw=1.0,color=groupColorS[j],marker='None')

    ax1.errorbar(volt_theory,GC_cap,yerr=None,ls=':',lw=1.0,color='k',marker='None')#,label =r'${\rm GC}$')
    ax1.set_ylabel(r'$\Sigma / \Sigma_{\rm ref}$',fontsize=10.)
    ax1.set_xlabel(r'$(\phi_0 - \phi_{\rm B})/ \phi^{\rm T}$',fontsize=10.)
    plt.setp(ax1.get_xticklabels(), fontsize=8.)
    plt.setp(ax1.get_yticklabels(), fontsize=8.)
    if Phix ==0.05:
        ax1.set_xlim(0,15.) 
        ax1.set_ylim(0,40.0)
#        plt.savefig('F1_ChargeVoltage_theory_'+str(Phix) + '.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=False,bbox_inches='tight')   
        ax1.set_xlim(0,10.) 
        ax1.set_ylim(0,8.0)
    elif Phix==0.17:
        ax1.set_xlim(0,20.) 
...
#        plt.savefig('F1_ChargeVoltage_theory_'+str(Phix) + '.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=False,bbox_inches='tight')   
        ax1.set_xlim(0,30.) 
        ax1.set_ylim(0,8.0)
    ax1.legend(loc='best',numpoints=1,prop=dict(size=8.),columnspacing=0.11,borderpad=0.25,labelspacing=0.1,ncol=1,handletextpad=0.07,markerscale=0.45)     
    fig.set_size_inches(3.37,2.5)
    plt.savefig('F1_ChargeVoltage_tot_'+str(Phix)+'.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=False,bbox_inches='tight')   
    plt.close()#	##Asym
...
#		print '\n\n',filename
    ...
    ax4.set_ylabel(r'$(\mu^{\rm EV}-\mu^{\rm EV}_{\rm B}) / k_{\rm B}T$',fontsize=9.)
...
#	    	plt.savefig('VoltRhosEV_'+str(np.round(np.mean(PhiWCABulkS),3))+'.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=False,bbox_inches='tight')      
    plt.close()
...
#    print "TEMPORARY FOR LAMMPS DEBUG"
	...
	### IN CASE IT COMES UP -- Np and Nm may or may not be switching... It's easy enough to fix when this code is uncomented
#    temp = N_plusS
#    N_plusS = N_minusS
#    N_minusS = temp
...
#	    print fail
	...
	##See P2.py to add back in any specific code...
	...
	#	    print fail
		    ...
		    print 'NEW: Plotting TOTAL Sigma versus Volt'
		    fig=plt.figure()
		    ax1=fig.add_subplot(111)
		    volt_theory = np.linspace(0,16)
		    GC_cap = 2*np.sinh(volt_theory/2.)
...
#		    fig.set_size_inches(3.37,3.0)
	...
	#	##Biksucks
		    ...
		    print 'NEW: Plotting Sigma_cor versus Volt_cor'
		    fig=plt.figure()
		    ax1=fig.add_subplot(111)
		    for (i,col) in enumerate(groupColorS):
	...
	#	    ax1.set_ylim(0,0.15)
		    ...
		    plt.savefig('F?_rhocor_vs_SigCor.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=True,bbox_inches='tight')   
	#	##Biksucks
		    plt.close()
...
#    ax1.errorbar(Phi_theory,-np.log(1-Phi_theory),yerr=None,color='k',ls='-.',label=r'$\mu^{\rm Bik}=-\ln(1-\Phi) $') #= -\ln (1-\Phi/0.65 )
    ...
    ax1.errorbar(Phi_theory,-np.log(1-Phi_theory/0.484),yerr=None,color='k',ls='-.',label=r'$\mu^{\rm Bik}_{\rm fit}(1.3 \sigma)$') #= -\ln (1-\Phi/0.65 )
    ...
    for line in WCAex:
  	...
  	exWCA.append(line[1])  
    ax1.plot(WCAPhi,exWCA,color='red',lw=1.3,ls='--',label=r'$\mu^{\rm WCA}$')  
...
#    labelS = [r'$ \sigma / \lambda_{\rm B} = 0.10$', r'$ 0.10 < \tilde \sigma \leq 0.2 $', r'$ 0.2 < \tilde \sigma \leq 1$', r'$ 1 < \tilde \sigma \leq 2$', r'$ 2 < \tilde \sigma \leq 10$', r'$   $', r'$   $']    	
    ...
    ax1.legend(loc='upper left',numpoints=1,prop=dict(size=8.),columnspacing=0.07,borderpad=0.25,labelspacing=0.08,ncol=2,handletextpad=0.05)#,handlelength=1.85)


#	    ax1.legend(loc='best',numpoints=1,prop=dict(size=8.),columnspacing=0.10,borderpad=0.15,labelspacing=0.1,ncol=2,handletextpad=0.05,markerscale=0.75)     

    ax1.set_xlabel(r'$\Phi_{\rm B} = \sigma^3 / \left ( 24 \lambda_{\rm B} \lambda_{\rm D}^2 \right ) $',fontsize=10.)
    ax1.set_ylabel(r'$\mu^{\rm EV}_{\rm B} / k_{\rm B} T$',fontsize=10.)
    plt.setp(ax1.get_xticklabels(), fontsize=8.)
    plt.setp(ax1.get_yticklabels(), fontsize=8.)
...
#    ax1.set_ylim(0,np.max(muexEV_bulkS)*1.1)
    ...
    fig.set_size_inches(3.37,2.5)
    plt.savefig('F1_BikSucks_Bulk.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=True,bbox_inches='tight')   
#    ax1.set_xlim(0,0.05805)
#    ax1.set_ylim(0,0.5) 
...
#    ax1.errorbar(Phi_theory,-np.log(1-Phi_theory/0.484),yerr=None,color='green',ls='-.',label=r'$\mu^{\rm Bik}_{\rm fit}(1.27 \cdot \sigma)$') #= -\ln (1-\Phi/0.65 )
    ...
    ax1.set_ylabel(r'$\mu^{\rm EV} \left [ 3  < z/\sigma < 6 \right ] / k_{\rm B} T$',fontsize=10.)
    plt.setp(ax1.get_xticklabels(), fontsize=8.)
    plt.setp(ax1.get_yticklabels(), fontsize=8.)
#    ax1.legend(loc='best',numpoints=1,prop=dict(size=8.),columnspacing=0.07,borderpad=0.25,labelspacing=0.1,ncol=1,handletextpad=0.05)
    ...
    plt.savefig('F1_BikSucks_mid.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=True,bbox_inches='tight')   
...
#    HSPhi_theory = Phi_theory*0.954028946**3
    ...
    ax1.set_ylabel(r'$\mu^{\rm EV} \left [ z < 3 \sigma \right ] / k_{\rm B} T$',fontsize=10.)
    plt.setp(ax1.get_xticklabels(), fontsize=8.)
    plt.setp(ax1.get_yticklabels(), fontsize=8.)
    ...
    ax1.set_ylim(0,20)
    fig.set_size_inches(3.37,2.25)
    plt.savefig('F1_BikSucks_wall.png', dpi=1600, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None, transparent=True,bbox_inches='tight')   
##Biksucks
    plt.close()
